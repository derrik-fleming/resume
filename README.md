# Resume

## Derrik Fleming

![Profile](https://img.shields.io/badge/Sr_Software_Engineer-🚀-blue)
![Last Updated](https://img.shields.io/badge/Last_Updated-March_27,_2025-green)
[![Pipeline Status](https://github.com/derrik-fleming/resume/actions/workflows/ci.yaml/badge.svg)](https://github.com/derrik-fleming/resume/actions)

This repository houses my professional resume, detailing my journey as a Software Engineer with a focus on full-stack development, cloud solutions, and mobile apps.

## Overview

I’m a problem-solver at heart, skilled in crafting robust applications with tools like AWS, TypeScript, and Flutter. This resume captures my experience, skills, and passion for building impactful software.

## Resume File

- **Location**: [resume.md](./resume.md)
- **Format**: Markdown for easy viewing on GitHub or locally.

### Viewing Options

- **GitHub**: Click the link above to see it rendered in your browser.
- **Local**: Clone and explore
- **Published Versions**:
  - [HTML](https://derrik-fleming.github.io/resume/resume.html)
  - [PDF](https://derrik-fleming.github.io/resume/resume.pdf)

## Get in Touch

- Email: [derrik.fleming@gmail.com](derrik.fleming@gmail.com)

Open to opportunities, collaborations, or a good tech chat!

## License

This content is personal and not for redistribution without my consent.

*Code, coffee, and curiosity* – that’s me in a nutshell.
